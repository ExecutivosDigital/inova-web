...
import { useChatPage } from "./components/chat-history-handler";
import { Section } from "./components/SectionGemini";
...
export default function BranchesList() {
  ...
  return (
    <>
                      ...
                      {prompts
                        ...
                        .map((sub, subIndex) => (
                          <button
                            ...
                            key={subIndex}
                            className="flex w-full flex-row items-center rounded-md p-2 text-start text-base hover:bg-[#4C785D]"
                          >
      ...
      <div className="flex w-full flex-row-reverse justify-center gap-2">
        <div className="flex h-[calc(100vh-150px)] w-full flex-col justify-between rounded-2xl bg-white p-2 xl:w-9/12 xl:p-4">
          <div className="relative">
            <div className="flex flex-row items-center gap-4">
              <Image
                ...
                src={"/logo/icon.png"}
              />
              <h1 className="text-2xl font-bold xl:text-2xl">InovAI</h1>
            </div>
            <h2 className="mt-1 text-lg font-medium xl:text-xl"></h2>
            {/* <div className="absolute top-2 right-2 flex items-center gap-2">
              <button
                onClick={() => setOpen(true)}
                className="bg-secondary flex items-center gap-2 rounded-full px-2 py-1 text-base xl:top-4 xl:right-4 xl:px-4 xl:py-2 xl:text-lg"
              >
                {selectedAi} <ChevronDown />
              </button>
              <button
                onClick={() => setOpenInfo(true)}
                className="bg-secondary flex items-center justify-center rounded-full p-1  xl:hidden"
              >
                <Plus />
              </button>
            </div> */}
          ...
          {/* {!selectedPrompt ? (
            <div className="flex flex-1 flex-col items-center justify-center gap-4 font-semibold">
              ...
              <button
                onClick={() => setOpen(true)}
                className="bg-secondary rounded-full p-2 px-8 text-lg "
              >
                Escolher categoria
              </button>
            </div>
        ...
        <div
          className={cn(
            "relative h-[calc(100vh-180px)] w-3/4 flex-col justify-between rounded-2xl border border-zinc-200 bg-white shadow-sm lg:w-1/2 xl:flex xl:w-3/12",
            openInfo ? "absolute right-0 flex" : "hidden",
          )}
        >
          <button
            onClick={() => setOpenInfo(false)}
            className="bg-secondary absolute top-4 right-[22px] z-10 flex items-center justify-center rounded-full p-1 xl:hidden"
          >
            <Minus />
          </button>
          <div className="flex h-full flex-col justify-between gap-4 pb-8">
            <div className="flex h-full flex-col">
              ...
              <button
                ...
                className="bg-secondary mt-4 h-12 w-2/3 cursor-pointer self-center rounded-3xl text-lg font-semibold"
              >
                Novo Chat
              </button>
            </div>
