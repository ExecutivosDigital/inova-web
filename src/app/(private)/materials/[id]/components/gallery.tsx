...
import MaterialMediaModal from "./MaterialMediaModal";

export function Gallery() {
  const [materialPages] = useState<number>(1);
  const [currentPage, setCurrentPage] = useState<number>(1);
  const [openMaterialMediaModal, setOpenMaterialMediaModal] =
  ...
  return (
    <>
        ...
        <div className="flex h-96 w-full flex-col">
          <ScrollArea className="h-80 w-full">
            {Array.from({ length: 10 }).map((_, i) => (
              <div
                key={i}
                onClick={() => setOpenMaterialMediaModal(true)}
      ...
      {openMaterialMediaModal && (
        <MaterialMediaModal
          open={openMaterialMediaModal}
          onClose={() => setOpenMaterialMediaModal(false)}
        />
      )}
    </>
  );
