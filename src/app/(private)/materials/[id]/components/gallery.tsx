...
import MaterialMediaSheet from "./MaterialMediaSheet";

export function Gallery() {
  const [materialPages] = useState<number>(1);
  const [currentPage, setCurrentPage] = useState<number>(1);
  const [openMaterialMediaSheet, setOpenMaterialMediaSheet] =
  ...
  return (
    <>
        ...
        <div className="flex h-96 w-full flex-col">
          <ScrollArea className="h-80 w-full">
            {Array.from({ length: 10 }).map((_, i) => (
              <div
                key={i}
                onClick={() => setOpenMaterialMediaSheet(true)}
      ...
      {openMaterialMediaSheet && (
        <MaterialMediaSheet
          open={openMaterialMediaSheet}
          onClose={() => setOpenMaterialMediaSheet(false)}
        />
      )}
    </>
  );
